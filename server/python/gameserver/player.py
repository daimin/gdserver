...
class Player(db.Db):
    UNLOGIN_STATUS = 0  # 未登陆
    NORMAL_STATUS = 1   # 已经登陆
    WAITING_STATUS = 2  # 等待游戏中
    PLAYING_STATUS = 2  # 游戏中
    ...
    def __init__(self, **kwargs):
        ...
        self.status = kwargs.get('status', Player.NORMAL_STATUS)
    ...
    def save(self):
        if not self.id:
            u = Player.find_user_by_name(self.name)
            if not u:
    ...
    def find_user(id):
        conn = db.Db.get_conn()
        result = db.Db.return_result(conn.get("SELECT * FROM `%s` WHERE `id` = '%s'" % (Player.tabname, id)))
        if result:
    ...
    def find_user_by_name(name):
        conn = db.Db.get_conn()
        result = db.Db.return_result(conn.get("SELECT * FROM `%s` WHERE `name` = '%s'" % (Player.tabname, name)))
        if result:
            return Player(**result)
...
if __name__ == "__main__":
    ...
    u = Player(name='戴敏', passwd=comm.md5('1234'), sex=1, score=100, conn=db.Db.get_conn())
    u.save()
