...
class GameServer(server_gevt.JBServer):
    ...
    def send_message(self, sock, msg_):
        if msg_.TID < protocol.ERR_NONE:
            super(GameServer, self).do_send_message(sock, msg_.TID, msg_.data, msg_.echo)
        else:
            msg_.echo = msg_.data  # 可能默认是填写的data变量
            super(GameServer, self).do_send_message(sock, msg_.TID, None, msg_.echo)
...
if __name__ == '__main__':
    ...
    GameServer().runserver(config.get('host', '0.0.0.0'), config.get('port', 5005))
